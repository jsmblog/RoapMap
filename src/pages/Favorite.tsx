...
const Favorite: React.FC = () => {
    ...
    try {
      const todosLosIds =
        currentUserData.favorites?.map(
          (favorite: FavoriteItem) => favorite.id
        ) || [];
  ...
  const ConfirmDeletFavorites = async () => {
    try {
      const favFilter = currentUserData.favorites.filter(
        (fav: FavoriteItem) => !isDeleteAll.includes(fav.id)
      );
      const refDocument = doc(db, 'USERS', currentUserData.uid);
      await updateDoc(refDocument, { fav: favFilter });

      showAlert(
        t('favorite.deletedTitle'),
        t('favorite.deletedMessage')
      );
      ...
      <IonHeader className="edit-profile-hearder">
        <IonToolbar className="setting-toolbar tema-oscuro">
          ...
          <IonTitle className="settings-ion-title texto-quinto">
            {t('favorite.title')} ({currentUserData.favorites?.length || 0})
          </IonTitle>
          {btnAction && (
            <IonButton
        ...
        currentUserData.favorites.length === 0 ? (
          ...
          <IonList className="favorites-list tema-oscuro2">
            <>
              ...
              {currentUserData.favorites.map(
                (favorite: FavoriteItem, index: number) => (
                  <IonItem
                    key={index}
                  ...
                  >
                    <div className="header-content">
                      <div className="place-info">
                        <IonIcon
                          className="category-icon texto-quinto"
